...
const makeTablerIcons = function() {
  fs.readdir(path.resolve('tabler-icons/icons'), function(err, files) {
    ...
    fs.writeFileSync(path.resolve('rc-neumorphism/src/components/Icon/svg/index.js'), text);
    ...
    _files.map(file => {
      fs.readFile(path.resolve(`tabler-icons/icons/${file}.svg`), function(err, linkString) {
        ...
        fs.writeFileSync(path.resolve(`rc-neumorphism/src/components/Icon/svg/${file}.js`), component);
      })
    });
  });
};
